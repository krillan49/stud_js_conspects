...
for (var i = 0; i < 10; i++) { // все условия и переменные цикла в круглых скобках после ключевого слова for
...
// С другими условиями
for (let m = 100; m > 5; m /= 2) {
  console.log(m); //=> 100 50 25 12.5 6.25
}

// С флоат переменной
for (let m = 1.1; m < 2; m += 0.2) {
  console.log(m); //=> 1.1 1.3 1.5 1.7 1.9
}

// Можно задавать несколько переменных в условии(через запятую), несколько условий для завершения итерации(через запятую) и отдельные условия изменения для переменных(через запятую). Насчет сложных условий завершения с операторами || или && - хз работает странно
let n = 36;
for(let i = 1, s = Math.sqrt(n); i <= s, s > 4; i++, s--) {
  console.log([i, s]); //=> [ 1, 6 ] => [ 2, 5 ]
}
...
// Варианты краткой записи если в теле цикла только одна строка кода
for (let i = 0; i < 10; i++)
	console.log(i);
...
// for..of (новый в ES6). Может просматривать все значения массива (без доступа к ним через их индекс).
let arr = ["one","two","three"];
...
// Обычно цикл While используется для булевых условий:
let isHasCar = true;
while (isHasCar) {
  // какойто код
  console.log('while');
...
// Do While будет обязательно выполнен один раз, а дальше уже будет проверять условие
var x = 13;
do {
	x--;
...
// break и continue можно использовать в любом цикле

// break    - оператор, служит для выхода из цикла полностью
// continue - оператор, пропускает лишь одну итерацию и не выходит из цикла.
