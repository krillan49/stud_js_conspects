...
new RegExp("abc","g");
/abc/g;
// g - глобальное соответствие (ищет все соответсвия, а не только 1е)
...
// source - свойство объектов регулярного выражения для получения строкового представления регулярного выражения.
let regex = /abc|def|ghi/;
regex.source; //=> "abc|def|ghi"


// Регулярное выражение не соответствующее ничему (символ после конца строки)
/$a/
...
// test() - метод используется для проверки того, соответствует ли строка объекту RegExp. Возвращает логическое значение: true/false. Не применяет флаг "g"
/ab/.test("abc");  //=> true
/ac/.test("abc");  //=> false
/^a.c$/.test("abc");   //=> true
/^a.c$/.test("abbc");  //=> false
...
// 1. match() - ищет подстроки соответсвующие регулярному выражению и выводит в видн массива.
// Если результатов больше одного или задан флаг "g", выходные данные содержат все совпадающие подстроки; Если результат один и нет флага "g", выходные данные содержат совпадающую подстроку, ее индекс и исходную строку. 
// Если результата совпадения нет, вывод будет null.
...
str.replace(/Hello/g,"I love");     //=> "I love World! I love CodeWars! "
"abcabc".replace(/^./,"A");         //=> Abcabc
"abcabc".replace(/c$/,"C");         //=> abcabC

// Принимает функцию 2м параметром для изменения совпадающей подстроки
"my name is John".replace(/\b./g, x => x.toUpperCase());  //=> "My Name Is John"
// поддерживает индексы
"this is an example".replace(/\b\w/g, (x, i) => i);       //=> '0his 5s 8n 11xample'
// элементы группировки в отдельных переменных
'abc'.replace(/(a)(b)(c)/, (a,b,c) => c+'0'+a+'1'+b+'2'); //=> 'b0abc1a2'
...
// Способ сделать соответсвия не жадными, тоесть останавливающимися при 1м найденном соответсвии
...
// \num (num — число с одной или двумя цифрами) - являются обратными ссылками, если им предшествует хотябы одно подвыражение (). Если есть более одного подвыражения, \1 первое подвыражение, \2 второе подвыражение итд.

"food foot green glass".match(/(.)\1/g);           //=> [ 'oo', 'oo', 'ee', 'ss' ]    // 2 одинаковых последовательных символа
"abba baab green glass roof".match(/(.)(.)\2\1/g); //=> [ 'abba', 'baab' ] // соотв 2м символам и их реверсу

// В параметрах для замены подвыражение записывается через синтаксис "$nun" вместо "\num"
...
// Отсчет нумерации подвыражений начинается по порядку открывающей скобки слева-направо. Например тут в примере обратные ссылки находящиеся в подвыражении используют нумерацию с 2х, тк \1 тут это подвыражение которое содержит другие подвыражения
var regex=/(\b(\w)(\w)?(\w)?\4\3\2)\b|(\b(\w)(\w)?(\w)?(\w)\8\7\6\b)/g
...
// Pегулярные выражения имеют четыре типа утверждений: положительное утверждение просмотра вперед, отрицательное утверждение просмотра вперед, положительное утверждение просмотра назад, отрицательное утверждение просмотра назад. Но в JS есть только два вида:
...
str.match(/(?=java)\b\w+script\b/g);  //=> [ 'javascript' ]   // только те соответсвия перед которыми есть 'java'(включительно)
str.match(/(?!java)\b\w+script\b/g);  //=> [ 'coffeescript' ] // только те соответсвия перед которыми нет 'java'
