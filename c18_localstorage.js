...
function localst() {
  ...
  var x = window.localStorage.getItem('score'); // похоже на hh['score']. Получаем значение из localStorage по ключу 'score'(название ключа можно придумать любое) и передаем его в переменную. (До определения(при самом первом нажатии) будет возвращать null).
  ...
  window.localStorage.setItem('score', x); // похоже на hh['score'] = x. Передается 2 параметра 1й ключ, 2й значение.
