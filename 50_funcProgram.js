...
class Container { // Класс контейнер объектами которого будут объекты-контейнеры
  ...
  // Возвращать значения свойства тоже будем не напрямую, а через метод, например в конце цепочки методов
  join() {
    return this.$value;
  }
...
// Either - реализует условное ветвление(аналог if) или для обработки ошибок, в зависимости от реальзации возвращется один из 2х подклассов: либо left (условие не выполнилось/произошла ошибка), либо right (условие выполнилось/не произошла ошибка)
...
// Далее применяем к этой монаде 'ap', тем самым мы передаем функции 'curriedSum' 1 арщумент, и засчет каррирования, в функции аргументе производной монады создалась новая фунция, которая уже ожидает не 2 аргумента, а 1.
...
// Далее идут монады - которые являются и аппликативными функторами и chain-функторы, тоесть реальзуют все(любые) из их методов
