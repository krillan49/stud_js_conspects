// ?? Поместить после функций и замыканий
...
while (!res.done) { // пример зачем нужен done
...
const createIterator = (arr) => {
  let i = 0;
  const arrIterator = {
    index: 0, // доп свойства для последнего индекса
    nextEl() {
      if (i < arr.length) {
...
// Совсем простой итератор прогоняющий массив кругами
...
// Адаптируем приведенный выше пример итератора выше. Поведение идентично, но эту реализацию гораздо проще писать и читать.
function* makeRangeIterator(start=0, end=Infinity, step=1) {
  let count = 0;
