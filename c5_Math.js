...
console.log(num1 ** 3); // возведение в степень
...
var a = parseInt('111.12'), b = parseFloat('111.12'), c = +'111.12'; // «+» — упрощенная форма parseFloat() (когда используется перед строкой)
console.log([a,b,c]) //=> [ 111, 111.12, 111.12 ]
...
// Число — это базовый тип данных в js. js также поддерживает числовые объекты. Объект является исходным значением объекта пакета. При необходимости JS автоматически выполняет преобразование между исходными данными и объектами. Вы можете явно создать объект Number с помощью конструктора Number().
...
var num = Number(value); // Объект Number имеет два общих свойства объекта JS: конструктор и прототип.
console.log(num); //=> 5
