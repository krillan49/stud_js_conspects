...
// ES5 - синтаксис функций:

// Функцию стандартного синтаксиса можно вызвать в любом месте кода, в том числе и выше того места где она задана
...
function test2(word=0, word2) {     // тут 0 - значение по умолчанию, на случай если будет передан undefined
...
// В ЖС и функция является особым объектом с типом 'function' и соответсвенно можно объявить для нее какое-то поле:
...
//                                            Анонимные фунции (ES5)

// Анонимную функцию (Функция без имени) можно присвоить в переменную. В отличие от стандартного синтаксиса можно вызвать только ниже того места где она задана (?? изза того что приваивается в переменную ??)

// ?? Тк это старый синтаксис то нужно использовать тип переменной var ??

var rev = function(arr) {  // присваивая функцию в переменную, создается функция с именем этой переменной
  return arr.reverse();
}
// Тк переменная возвращает функцию то можем применить (), чтобы ее вызвать
rev([1,2,3]); //=> [ 3, 2, 1 ]



//                                     Стрелочные функции (Анонимные фунции ES6)

// Синтаксис: (параметры) => {оператор} или выражение

// => - символ по сути заменяющий ключевое слово function

// Если с правой стороны стрелки одноcтрочное выражение, фигурные скобки можно опустить и не использовать оператор return
const plus = (a=0, b=0) => a + b; // значения по умолчанию пишутся стандартно
plus(3, 4); //=> 7

// Если слева от стрелки существует только один параметр, скобки можно опустить. .
const add = x => x + 1;

// Если фукция не принимает параметров, то скобки обязательны
...
// Тоесть можно передавать любую функцию аргументом во встроенные функции если число и тип параметров подойдут
...
//                                    Передача функции как параметра (коллбек)

// использоавание колбека - функции как параметра в другой функции
...
// возвращаем саму функцию через переменную с ее именем
arr.filter(notEmpty);                                      //=> [ 1, 2, 3 ]
...
// тоже самое что и
arr.filter(item => item !== undefined && item !== null);   //=> [ 1, 2, 3 ]
...
// Пример функции конструктора - тоесть функции которая создает другие функции
function createFn() {
  function greeeting() {
    console.log('hello world');
  }
  return greeeting; // возвращаем функцию без ее вызова
}
...
// Замыкания в ЖС:
// 1. Функции генерируют и возвращают новые функции
// 2. Возвращаемые функции помнят тот контекст где они были созданы

// При запуске функции-конструктора создается контекст вызова этой фунции с локальной оперативной памятью к ней относящейся, в которой создаются все переменные тела этой функции, в том числе и генерируемая функция в виде текстовой инструкции. Далее return возвращает наружу в глобальную область видимости сгенерированную функцию и присваиваем ее в переменную. Функция конструктор заканчивает свое выполнение, удалется из стека и очищается ее локальная память, но данные из нее идут в связке с возвращенной функцией, тк любая жс-функция это функция + объект и у него есть свойство прототипа и так же скрытые свойства, одно из которых называется "scope", в этом свойстве и хранятся все данные из контекста в котором эта функция создавалась, например переменные с их значениями. Далее когда мы вызываем возвращенную сгенеренную фунцию от новой переменной, она создает свой контекст со своей локальной памятью, но она обращается внутри себя к переменной из контекста функции конструктора, первым делом эта переменная ищется в текущей локальной памяти, но ее там нет и тогда эта переменная ищется в объекте функции под ключем scope

function init() {
  let name = "Mozilla"; // name - локальная переменная, созданная в теле функции init тоесть в ее бласти видтмости
