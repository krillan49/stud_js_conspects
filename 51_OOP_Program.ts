...
// Сокрытие - создание скрытуой части класса, например скрытые свойства и методы, которые нельзя вызвать вне тела класса. Для того чтобы делать методв или свойства публичными или приватными во многих языках существуют модификаторы доступа public и private
...
// Наследование - спозоб взаимодействия между классами, позволяет переиспользовать код, тк класс получает свойства и методы от родительского класса
...
// Полиморфизм - концепция ООП, которая позволяет одному и тому же фрагменту кода работать с разными типами данных. Может самый важный принцип ООП и на нем основаны почти все паттерны проектирования

// Выделяют 2 типа полимофизма: параметрический полиморфизм (истинный) и ad-hoc полиморфизм (мнимый)


// 1. ad-hoc полиморфизм (мнимый). TНапример в классе Calculator есть 2 метода, которые называются одинаково, принимают одинаковые параметры, но они разных типов, 1й метод принимает числовой тип, а 2й строковый. В итоге при вызове как бы одного и того же метода в зависимости от типов передаваемых параметров, произведем или сложение чисел или конкатинацию строк. Происходит засчет перегрузки методов.
class Calculator {
...
// Класс наследник наследует метод greeting, но переопределим его, чтобы он выводил другое
class Employee extends Person {
  private _pasport_number;
...
employee.greeting() //=> "Привет я работник, меня зовут Petya"
...
// Абстрактный класс - похож на интерфейс, в нем можно объявлять абстрактные методы, которые будут аналогом методов интерфейса, те без реализации, но при этом можно создавать и обычные методы с какой-то реализацией. Класс который унаследует от абстрактного, унаследует обычные методы и должен будет реализовать абстрактные методы. Абстрактные классы используются существенно реже интерфейсов
...
// Создадим класс с объектами которого бует работать класс имплементирующий интерфейс
class User {
  username: string;
  age: number;
}
...
// Создадим интерфейс Репозиторий с 4мя CRUD методами. Тк в классе в который имплементируем интерфейс методы ожидают аргументы, добавим в интерфейс обобщение/дженерик - в данном случае тип T это чтото общее, какойто тип данных приходящий извне
