...
// alert, prompt, confirm - это операторы встроенных функций
...
let result = confirm("Вы согласны с confirm?");
...
let info = prompt("Сколько вам лет?", 25); // Второй параметр это значение по-умолчанию, он не обязателен
...
if (confirm("Ты огромен?")) {
  let sword = prompt('Введи длинну своего меча');
  sword && sword > 9000 ? (alert('Крокер!!!')) : (alert(sword + ' - это маловато'));
} else {
  alert('ок');
}
